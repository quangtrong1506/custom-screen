...
import { IpcKey, SettingInterface, VideoInterface } from '../../types';
...
export const connectIpcMain = (mainWindow: Electron.BrowserWindow) => {
	...
	// lấy danh sách video
	ipcMain.on(IpcKey.getVideoList, _event => {
		sendListVideos(mainWindow);
	});

	// cài nền
	ipcMain.on(IpcKey.getBackground, async _event => {
		try {
			const settingsPath = path.join(app.getPath('userData'), 'config', 's.bak');
			let data = (await readJsonFile<SettingInterface>(settingsPath)) || initSettings;
	...
	ipcMain.handle(IpcKey.getShortcuts, async () => {
		try {
			const settingsPath = path.join(app.getPath('userData'), 'config', 's.bak');
			const data = (await readJsonFile<SettingInterface>(settingsPath)) || initSettings;
	...
	ipcMain.on(IpcKey.closeMainWindow, async _event => {
		mainWindow.hide();
	});
